...
author: "María José Calvo y Daniel Ulloa"
date: "2020-10-07"
output:
  html_document:
    theme: readable    
...
```{r}
paises_asia = world[world$continent == "Asia" &
                    world$lifeExp >= 80  
                                                    ,]
...
# Se formatea la tabla con el nuevo data frame
kable(
  paises_asia2[, c("name_long","lifeExp"), drop = TRUE], 
  col.names=c("País","Esperanza de vida"),  
  caption="Datos de esperanza de vida de los países de Asia",
  
) %>%
kable_styling() 
```{r}
paises_asiaconNA = world[world$continent == "Asia" &
                    world$lifeExp >= 80  
                                                    ,]

paises_asia <- na.omit(paises_asiaconNA)

# Se formatea la tabla con el nuevo data frame
kable(
  paises_asia[, c("name_long","lifeExp"), drop = TRUE], 
  col.names=c("País","Esperanza de vida"),  
  caption="Datos de esperanza de vida de los países de Asia",
...
ggplot(paises_asia, aes(x=reorder(name_long, -lifeExp), y=    lifeExp)) +
  ggtitle("Esperanza de vida mayor a 80 años en Asia") +
  xlab("País") + 
...
```{r}
paises_oceania = world[world$continent == "Oceania" ,]
plot(
  paises_oceania["lifeExp"], 
...
```{r}
paises_centroamerica = centro_america[centro_america$lifeExp >= 74  
                                                    ,]
plot(
  paises_centroamerica["lifeExp"], 
  main="Esperanza de vida en Centroamérica"
)
...
```{r}
paises_suramerica = world[world$continent == "South America"                            ,]
paisesSAarea=paises_suramerica[paises_suramerica$area_km2 >= 2500000 ,]
plot(
  paisesSAarea["area_km2"], 
  main="
  Países de América del Sur con área de 
  al menos 2.5 millones de km2 "
)
paisesSApop=paises_suramerica[paises_suramerica$pop >= 40000000 ,]
plot(
  paisesSApop["pop"], 
  main="
  Países de América del Sur con población de 
  al menos 40 millones de personas"
)
